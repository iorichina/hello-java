...
public class IPExt {
    private static final String RESOURCE_PATH = "/17monipdb.datx";
...
//        healthCheckRegistry.register("deadlocks", new ThreadDeadlockHealthCheck());
    ...
    public static void init() throws Exception {
        ...
        synchronized (IPExt.class) {
            if (initialized) {
                return;
            }
            URL url = IPExt.class.getResource(RESOURCE_PATH);
            if (null == url) {
                return;
            }
            setFilename(url.getPath());
            IPExt.load(filename, true);
            IPExt.watch();
            setInitialized(true);
        }
    }
